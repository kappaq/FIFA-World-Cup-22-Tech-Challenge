...
def detailed_offside():
    ...
    df.to_csv('selenium_whoscored_scrap_player_stats_detailed_offside.csv', header=True, index=False)


def detailed_clearances():
    df = get_pages_data(6, 6, 69, driver)
    df.columns = ['No.', 'Player', 'Apps', 'Mins', 'Total', 'Rating']
    df.to_csv('selenium_whoscored_scrap_player_stats_detailed_clearances.csv', header=True, index=False)


def detailed_blocks():
    df = get_pages_data(8, 6, 69, driver)
    df.columns = ['No.', 'Player', 'Apps', 'Mins', 'ShotsBlocked', 'CrossesBlocked', 'PassesBlocked', 'Rating']
    df.to_csv('selenium_whoscored_scrap_player_stats_detailed_blocks.csv', header=True, index=False)


def detailed_saves():
    df = get_pages_data(9, 6, 69, driver)
    df.columns = ['No.', 'Player', 'Apps', 'Mins', 'Total', 'SixYardBox', 'PenaltyArea', 'OutOfBox', 'Rating']
    df.to_csv('selenium_whoscored_scrap_player_stats_detailed_saves.csv', header=True, index=False)
...
df = pd.read_csv("selenium_whoscored_scrap_player_stats_detailed_offside.csv", sep=",")
...
df = pd.read_csv("selenium_whoscored_scrap_player_stats_detailed_clearances.csv", sep=",")
...
df = pd.read_csv("selenium_whoscored_scrap_player_stats_detailed_blocks.csv", sep=",")
...
df = pd.read_csv("selenium_whoscored_scrap_player_stats_detailed_saves.csv", sep=",")
