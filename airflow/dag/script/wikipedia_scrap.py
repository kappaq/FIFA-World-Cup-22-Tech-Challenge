...
df_coach.to_csv('df_coach.csv', header=False)
...
cs.execute('CREATE OR REPLACE TABLE "wikipedia_scrap_players" ( "No." INTEGER, "Pos." STRING, "Player" STRING, "Date of birth (age)" STRING, "Caps" STRING, "Goals" STRING, "Club" STRING)')
cs.execute('CREATE OR REPLACE TABLE "wikipedia_scrap_coach" ( "No." INTEGER, "Team" STRING, "Captain" STRING, "Coach" STRING)')


df_players = pd.read_csv("wikipedia_scrap_team_clubs.csv", sep=",")
df_coach = pd.read_csv("df_coach.csv", sep=",")

write_pandas(ctx,df_players,table_name="wikipedia_scrap_players")
write_pandas(ctx,df_coach,table_name="wikipedia_scrap_coach")